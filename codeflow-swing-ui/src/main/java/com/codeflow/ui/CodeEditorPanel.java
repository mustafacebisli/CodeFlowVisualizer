...
public class CodeEditorPanel extends JPanel {
    ...
    /** Parse sonrası kısa uyarı (ör. tip bulunamadı); boşsa yalnızca satır sayısı */
    ...
    private EditorFindDialog findDialog;
    ...
    public CodeEditorPanel() {
        ...
        title.setForeground(AppTheme.ACCENT_GREEN);
        header.add(title, BorderLayout.WEST);

        statusLabel = new JLabel("0 sat\u0131r");
        statusLabel.setFont(AppTheme.sans(Font.PLAIN, 11));
        ...
        toolbar.setBackground(AppTheme.TOOLBAR_BG_ALT);

        JLabel classLabel = new JLabel("S\u0131n\u0131f:");
        classLabel.setFont(AppTheme.sans(Font.PLAIN, 11));
        classLabel.setForeground(AppTheme.TEXT_SUBTLE);
        ...
        textArea.setFont(AppTheme.codeEditor());
        textArea.setTabSize(4);
        textArea.setBackground(AppTheme.EDITOR_BG);
        textArea.setForeground(AppTheme.TEXT_PRIMARY);
        textArea.setCaretColor(Color.WHITE);
        textArea.setSelectionColor(AppTheme.SELECTION_BG);
        ...
        im.put(KeyStroke.getKeyStroke(KeyEvent.VK_F, InputEvent.CTRL_DOWN_MASK), "codeflowFind");
        am.put("codeflowFind", new AbstractAction() {
            @Override
            public void actionPerformed(ActionEvent e) {
                Window w = SwingUtilities.getWindowAncestor(CodeEditorPanel.this);
                if (w == null) return;
                if (findDialog == null) {
                    findDialog = new EditorFindDialog(w, textArea);
                }
                findDialog.openWithSelection();
            }
        });
    }
    ...
    /** {@code null} veya boş: uyarıyı temizler. */
    public void setParseHint(String hint) {
        this.parseHint = hint != null ? hint.trim() : "";
        updateStatus();
    }
    ...
    private void updateStatus() {
        ...
        String base = lines + " sat\u0131r";
        if (!parseHint.isEmpty()) {
            statusLabel.setForeground(AppTheme.HINT_AMBER);
            statusLabel.setText(base + "  \u00b7  " + parseHint);
        } else {
            statusLabel.setForeground(AppTheme.TEXT_MUTED);
    ...
    private static class LineNumberPanel extends JPanel {
        ...
        LineNumberPanel(JTextArea textArea) {
            ...
            setBackground(AppTheme.LINE_NUMBER_BG);
            setFont(AppTheme.mono(Font.PLAIN, 11));
        ...
        protected void paintComponent(Graphics g) {
            ...
            for (int i = 0; i < lineCount; i++) {
                ...
                g2.setColor(AppTheme.LINE_NUMBER);
