...
public class FlowchartRenderer extends JPanel {
    ...
    private static final Color BG = AppTheme.CANVAS_BG;
    private static final Color ARROW_COLOR = AppTheme.ARROW;
    private static final Color PROCESS = AppTheme.PROCESS_NODE;
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (multiMethods != null && !multiMethods.isEmpty()) {
            ...
            for (CodeMethod cm : multiMethods) {
                ...
                drawCapsule(g2, cx, y, "Basla", AppTheme.SUCCESS_GREEN);
                y += NODE_H;
                y = drawNodeList(g2, cm.getFlowNodes(), cx, y);
                drawArrowDown(g2, cx, y, cx, y + V_GAP);
                y += V_GAP;
                drawCapsule(g2, cx, y, "Son", AppTheme.ERROR_RED);
    ...
    private int drawMethodSectionHeader(Graphics2D g2, CodeMethod cm, int y) {
        ...
        g2.setColor(AppTheme.SECTION_BAR);
        g2.fill(bar);
        g2.setColor(AppTheme.SECTION_BAR_EDGE);
        ...
        g2.setColor(AppTheme.SECTION_TITLE);
    ...
    private int drawConditionBranch(Graphics2D g, FlowNode node, int cx, int y) {
        ...
        if (hasTrue) {
            ...
            drawLabel(g, cx - NODE_W / 2 - 14, y + DIAMOND_H / 2 - 6, "T", AppTheme.SUCCESS_GREEN);
        ...
        if (hasFalse) {
            ...
            drawLabel(g, cx + NODE_W / 2 + 14, y + DIAMOND_H / 2 - 6, "F", AppTheme.ERROR_RED);
            ...
            drawLabel(g, cx + 10, diamondBottom + V_GAP / 2 - 2, "F", AppTheme.ERROR_RED);
    ...
    private int drawLoopBranch(Graphics2D g, FlowNode node, int cx, int y) {
        ...
        if (!node.getTrueBranch().isEmpty()) {
            ...
            g.setColor(AppTheme.withAlpha(AppTheme.WARNING_AMBER, 140));
    ...
    private void drawCapsule(Graphics2D g, int cx, int y, String text, Color color) {
        ...
        g.setColor(AppTheme.withAlpha(color, 35));
        g.fill(shape);
        g.setColor(color);
    ...
    private void drawDiamond(Graphics2D g, int cx, int y, String text) {
        ...
        d.lineTo(cx - w / 2, my);
        d.closePath();
        Color c = AppTheme.WARNING_AMBER;
        g.setColor(AppTheme.withAlpha(AppTheme.WARNING_AMBER, 30));
        g.fill(d);
        g.setColor(c);
        g.setStroke(new BasicStroke(2));
        g.draw(d);
    ...
    private void drawRoundRect(Graphics2D g, int cx, int y, String text, Color c) {
        int x = cx - NODE_W / 2;
        g.setColor(AppTheme.withAlpha(c, 25));
